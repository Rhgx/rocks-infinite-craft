...
final class RecipeStyleEntry extends AbstractConfigListEntry<Integer> {
    ...
    RecipeStyleEntry(String title, int initial, List<String> labels, Consumer<Integer> save) {
        ...
        slider = new AbstractSliderButton(0, 0, 106, 20, Component.literal(labels.get(selected / 25)), selected / 100.0) {
            @Override protected void setValue(double next) {
                super.setValue(Math.round(Math.clamp(next, 0, 1) * 4) / 4.0);
            }
            @Override protected void applyValue() { selected = (int) Math.round(value * 4) * 25; }
            @Override protected void updateMessage() { setMessage(Component.literal(labels.get(selected / 25))); }
    ...
    @Override public Integer getValue() { return selected; }
    @Override public Optional<Integer> getDefaultValue() { return Optional.of(50); }
    @Override public boolean isEdited() { return selected != original; }
    @Override public int getItemHeight() { return 26; }
    @Override public List<? extends GuiEventListener> children() { return List.of(slider, reset); }
    @Override public List<? extends NarratableEntry> narratables() { return List.of(slider, reset); }
