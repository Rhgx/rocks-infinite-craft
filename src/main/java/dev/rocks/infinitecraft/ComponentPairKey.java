...
final class ComponentPairKey {
    /** A world-seeded coin toss stays the same when ingredients are swapped or a request is retried. */
    static boolean firstWins(JsonElement first, JsonElement second, long worldSeed) {
    ...
    static String of(String firstId, String secondId, JsonElement first, JsonElement second) {
        String a = canonical(first).toString(), b = canonical(second).toString();
